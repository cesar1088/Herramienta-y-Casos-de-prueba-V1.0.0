/*-----------------------------------------------------------------------------
| Caso de Prueba #4: "Generación de Variables Aleatorias Continuas"(A1)(FLOAT) |
...
@WebService()
public class A1_FContinuas
{
    @WebMethod(operationName = "peticionContinuasFA1")
    ...
    @WebParam(name = "tipoAleatorios") int tipoAleatorios)
    {
        ...
        try
        {
            ...
            float[] generadosX = new float[repeticiones];
            float[] valores_rn = new float[repeticiones];

            //Métodos Congruenciales
            float X0, X1; //semillas
            float rn; //variables obtenidas
            float num, den; // Fracciones
            float xi; //Congruecial aditivo
            float varianza, u, p1, p2; //Distribucion Gauss
            float x, n; //Distribucion Gamma
            float facAlpha, factBeta, factAlphaBeta; //Distribucion Beta
            float D, Y0, posDer, posIzq, alObtenido, tamañoLateral; //Valores medios
                ...
                case 0: //Si se seleccionaron los Métodos Congruenciales...
                {
                    for (int index = 0; index <= 3; index++)
                    {
                        ...
                        for (int i = 0; i < repeticiones; i++)
                        {
                            ...
                            den = (float)(n * Math.pow(beta, alpha));
                        ...
                        switch (index)
                        {
                            case 4: //Método de los Cuadrados Medios
                            {
                                for (int i = 0; i < repeticiones; i++)
                                {
                                    ...
                                    alObtenido = (float)(Double.parseDouble(valorIzq + valorDer));
                                    X0 = (int) alObtenido;
