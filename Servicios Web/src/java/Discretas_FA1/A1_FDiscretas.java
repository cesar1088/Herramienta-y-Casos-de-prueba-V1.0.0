/*-----------------------------------------------------------------------------
| Caso de Prueba #5: "Generación de Variables Aleatorias Discretas" (A1)(FLOAT)|
...
@WebService()
public class A1_FDiscretas
{
    @WebMethod(operationName = "peticionDiscretasFA1")
    public double peticionDiscretasFA1(@WebParam(name = "repeticiones") int repeticiones)
    {
        ...
        try
        {
            ...
            float q = 1 - p;
            float a, b;
            float d1, d2, numerador, denominador, combinacion;

            float valorBinomial; //Binomial
            float r, binomialNeg; //Binomial Negativo
            float e, landa, valorPoisson; //Poisson
            float valorGeometrica; //Geometrica
            float N, COMB1, COMB2, COMB3, hiperGeom; //HiperGeometrica
            float bernoulli; //Bernoulli
            float fn, multinomial; //Multinomial
            float[] X, P;  //Multinomial
            float x, uniforme; //Uniforme discreta
            ...
            for (int i = 0; i < repeticiones; i++, k++)
            {
                ...
                valorBinomial = (float)(combinacion * Math.pow(p, k) * Math.pow(q, n - k));
            ...
            for (int i = 0; i < repeticiones; i++)
            {
                ...
                k = (float)contador + i; //Termina el cálculo de x
                ...
                binomialNeg = (float)(combinacion * Math.pow(p, r) * Math.pow(q, k - r));
            ...
            for (int i = 0; i < repeticiones; i++, k++)
            {
                ...
                valorPoisson = (float)(Math.pow(e, landa * -1) * (Math.pow(landa, k) / factorial));
            ...
            n = 4;
            X = new float[] {3, 0, 1};
            P = new float[] {(float)0.5, (float)0.3, (float)0.2};
            ...
            for (int i = 0; i < repeticiones; i++, n += 0.5)
            {
                ...
                a = (float)(fn / (fX0 * fX1 * fX2));
                b = (float)(Math.pow(P[0], X[0]) * Math.pow(P[1], X[1]) * Math.pow(P[2], X[2]));
                multinomial = a * b;
